...
use leafwing_input_manager::{
    action_state::ActionState, plugin::InputManagerPlugin, Actionlike, InputControlKind,
};

use crate::{GameplaySet, Player};
...
fn apply_camera_controls(
    ...
    if let Ok((camera_entity, projection, action_state, mut camera)) = camera_query.get_single_mut()
    {
        ...
        if let Some(zoom_input) = action_state.axis_data(&CameraAction::Zoom) {
            if let Projection::Perspective(projection) = projection.into_inner() {
                projection.fov = (projection.fov + ZOOM_SPEED * delta * -zoom_input.value)
