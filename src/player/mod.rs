...
impl Plugin for PlayerPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(
            Update,
            (
                (move_player).in_set(GameplaySet),
...
fn move_player(
    ...
    if let Ok((transform, mut controller, action_state)) = player_query.get_single_mut() {
        if let Some(walk_input) = action_state.dual_axis_data(&PlayerAction::Walk) {
            ...
            controller.basis(TnuaBuiltinWalk {
                desired_velocity: walk_dir.normalize_or_zero()
                    * PLAYER_WALK_SPEED
                    * ((*state.get() == PlayerState::Free) as u32 as f32),
                float_height: 1.5,
                ..Default::default()
            });
        }
...
fn handle_player_interaction(
    ...
    if let Ok((player_transform, action_state)) = player_query.get_single() {
